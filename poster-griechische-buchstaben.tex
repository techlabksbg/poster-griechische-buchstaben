...
%    Gross & Klein & Aussprache  & Gebrauch \\
    $A\,\,$  & $\alpha$ $\,\,\,$ & Alpha & \Large Winkel, $\alpha$-Strahlung (Heliumkerne) \\
    $B$ & $\beta$ & Beta & \Large Winkel $\beta$-Strahlung (Elektronen)\\
    $\Gamma$ & $\gamma$ & Gamma & \Large Winkel, $\gamma$-Strahlung (Photonen), Gravitationskonstante $\Gamma$, \\
    ...
    $H$ & $\eta$ & Eta & \Large \\
    $\Theta$ & $\theta$ & Theta & \Large \\
    $I$ & $\iota$ & Iota & \Large \\
    $K$ & $\kappa$ & Kappa & \Large Krümmungsmass $\kappa$\\
    $\Lambda$ & $\lambda$ & Lambda & \Large Streckungsfaktor $\lambda$ (Geometrie), Wellenlänge $\lambda$\\
    $M$ & $\mu$ & Mü & \Large Mittelwert $\mu$, Massprefix für Millionstel: 1 $\mu$m = $10^{-6}$ m\\
    $N$ & $\nu$ & Nü & \Large Frequenz bei Wellen $\nu$ \\
    $\Xi$ & $\xi$ & Xi & \Large \\
    $O$ & $o$ & Omikron $\,\,$ & \Large \\
    $\Pi$ & $\pi$ & Pi & \Large Kreiszahl $\pi = 3.14159265358979\dots$, Produktzeichen $\prod$\\
    $P$ & $\rho$ & Rho & \Large Symbol für Dichte in der Physik.\\
    $\Sigma$ & $\varsigma$ & Sigma & \Large Summenzeichen $\sum$, Standardabweichung $\sigma$\\
    $T$ & $\tau$ & Tau & \Large Periodendauer $\tau$ einer Schwinung\\
...
\adjustbox{valign=M}{\small \sf (c) Ivo Blöchliger}
